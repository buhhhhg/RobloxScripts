...
if not Drawing then
    d = loadstring(game:HttpGet("https://raw.githubusercontent.com/buhhhhg/RobloxScriptz/main/drawing-solara.lua"))()
    task.wait(.1)
    if not Drawing and d then
        getgenv().Drawing = d
    end
    task.wait(.1)
end
...
local ui = {} do 
    ...
    do 
        ...
        w_ModListTitle.Text = ' '..'funnyClient '..REDLINEVER..' '
...
local function disablecons(signal, id)
    ...
    local gc = getconnections or function(...)
        warn("fake getconnections used! :(")
        return {}
    end

    local connections = gc(signal)
